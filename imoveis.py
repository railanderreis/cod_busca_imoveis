...
# %%
url = 'https://www.vivareal.com.br/venda/espirito-santo/vitoria/apartamento_residencial/?pagina={}'
# %%
i = 1
retorno = requests.get(url.format(i))
soup = bs(retorno.text)
# %%
houses = soup.find_all(
  'a',{'class': 'property-card__content-link js-card-title'})
qtd_imoveis = float(soup.find('strong', 
                      {'class':'results-summary__count'}).text.replace('.',''))  
# %%
house = houses[0]
# %%
df = pd.DataFrame(
...
while qtd_imoveis > df.shape[0]:
  ...
  for house in houses:
      ...
      try:
        ...
        condominio = None
      try:
        wlink = 'http://www.vivareal.com.br' + house['href']
      except:
        wlink = None      
